...
* dasginganinja/ci-build:8.3-apache-4
...
2. Build the image. 
    ...
    2. `docker image build -t dasginganinja/ci-build:8.3-apache-4 .`
3. Run the image locally for testing
    1. `docker run -it dasginganinja/ci-build:8.3-apache-4 bash`
4. Push the updated image (ONCE YOU ARE SURE) to Docker Hub
    1. `docker image push dasginganinja/ci-build:8.3-apache-4`
...
```
docker image build -t dasginganinja/ci-build:8.3-apache-4  --no-cache --progress=plain . 2>&1 | tee build.log
```
