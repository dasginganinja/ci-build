...
* dasginganinja/ci-build:8.4-apache-5
...
To build an image, change into the appropriate folder, i.e. `php8.4` and then run `docker image build - < Dockerfile`.

# Building an image for local tagging, testing, and release

1. Change into the directory you want to run things in, i.e. php8.3
    1. `cd php8.3`
2. Build the image. 
    1. Choose the appropriate tag from the list above and use that as the tag in your command
    2. `docker image build --platform=linux/amd64 -t dasginganinja/ci-build:8.4-apache-5 .`
3. Run the image locally for testing
    1. `docker run -it dasginganinja/ci-build:8.4-apache-5 bash`
4. Push the updated image (ONCE YOU ARE SURE) to Docker Hub
    1. `docker image push dasginganinja/ci-build:8.4-apache-5`

Note: You may need to `docker login` and enter your Docker Hub credentials. :)


# Debug Logs for builds
```
docker image build -t dasginganinja/ci-build:8.4-apache-5  --no-cache --progress=plain . 2>&1 | tee build.log
```
